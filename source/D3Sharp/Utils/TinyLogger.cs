...
namespace D3Sharp.Utils
{
    ...
    public class FileTarget : Target, IDisposable
    {
        ...
        {
            this._logStream.WriteLine(string.Format("[D3GS]: {2}", level.ToString().PadLeft(5), logger, message));
        }

        public override void LogException(Level level, string logger, string message, Exception exception)
        {
            this._logStream.WriteLine(string.Format("[D3GS]: {2} - [Exception] {3}", level.ToString().PadLeft(5), logger, message, exception));
        }
    ...
    public class ConsoleTarget : Target
    {
        ...
        public override void LogMessage(Level level, string logger, string message)
        {
            ...
            Console.WriteLine(string.Format("[D3GS]: {2}", level.ToString().PadLeft(5), logger, message));
        }

        public override void LogException(Level level, string logger, string message, Exception exception)
        {
            SetForeGroundColor(level);
            Console.WriteLine(string.Format("[D3GS]: {2} - [Exception] {3}", level.ToString().PadLeft(5), logger, message, exception));
        }
